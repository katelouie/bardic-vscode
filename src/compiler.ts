...
export interface CompilationError {
    type: 'python-not-found' | 'bardic-not-installed' | 'syntax-error' | 'timeout' | 'unknown';
    message: string;
    hint?: string;
    lineNumber?: number;
    filePath?: string;
...
export class BardCompiler {
    ...
    async compile(filePath: string): Promise<CompilationResult> {
        ...
        const preferCLI = config.get<boolean>('preferCLI', true);
     ...
     */
    private async compileWithCLI(filePath: string, pythonPath: string): Promise<CompilationResult> {
        return new Promise((resolve) => {
            const tempOutput = path.join(os.tmpdir(), `bardic-${Date.now()}.json`);
            const config = vscode.workspace.getConfiguration('bardic');
            const timeout = Math.min(config.get<number>('compilationTimeout', 10000), 60000);
     ...
     */
    private parseBardError(stderr: string, filePath: string): CompilationError {
    ...
    private convertToGraphFormat(storyData: any): CompiledStory {
        ...
        const extractChoicesFromContent = (content: any[]): any[] => {
            const choices: any[] = [];
            if (!content) return choices;

            content.forEach((token: any) => {
                ...
                if (token.type === 'conditional' && token.branches) {
                    token.branches.forEach((branch: any) => {
        ...
        Object.keys(passages).forEach(passageName => {
            ...
            if (dynamicChoices.length > 0) {
                console.log(`[Bardic] Found ${dynamicChoices.length} dynamic choices in ${passageName}:`,
                    dynamicChoices.map((c: any) => c.target));
            }
            ...
            allChoices.forEach((choice: any) => {
                ...
                if (!edges.find(e => `${e.from}->${e.to}` === edgeKey)) {
                    ...
                    if (Array.isArray(choiceText)) {
                        ...
                        choiceText = choiceText.map((token: any) => {
     ...
     */
    private async compileWithSimpleParser(content: string, filePath: string): Promise<CompilationResult> {
    ...
    private wrapEdgeLabel(label: string, maxLength: number): string {
