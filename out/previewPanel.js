...
class PreviewPanel {
    static async createOrShow(extensionUri, passageName, storyData, pythonPath, initialState) {
        ...
        // Create new panel
        const panel = vscode.window.createWebviewPanel('bardicPreview', 'Bardic Preview', column, {
            enableScripts: true,
            retainContextWhenHidden: true
        });
        PreviewPanel.currentPanel = new PreviewPanel(panel, extensionUri, storyData, pythonPath);
        await PreviewPanel.currentPanel.previewPassage(passageName, initialState || {});
    }
    constructor(panel, extensionUri, storyData, pythonPath) {
        this._disposables = [];
        this._currentState = {};
    ...
    async startPythonProcess() {
        return new Promise((resolve, reject) => {
            ...
            this._pythonProcess = (0, child_process_1.spawn)(this._pythonPath, [scriptPath]);
            let stdout = '';
            let stderr = '';
            // Handle stdout
            ...
            this._pythonProcess.on('close', (code) => {
                ...
                if (code !== 0 && code !== null) {
                    this._panel.webview.html = this._getErrorHtml(`Preview process exited with code ${code}\n${stderr}`);
                }
            });
            // Handle spawn errors
    ...
    async previewPassage(passageName, state) {
    ...
    async handleChoiceSelected(index) {
        const command = {
            type: 'choice',
            index
        };
